#!/usr/bin/env python3
"""
Process Existing 114th Congress XML Files
Processes already downloaded XML files using proper XSL schemas and validation
"""
...
class Existing114thProcessor:
    ...
    def _setup_logging(self):
        ...
        logging.basicConfig(
            ...
            handlers=[
                ...
                logging.FileHandler("logs/existing_114th_processor.log", mode='a', encoding='utf-8')
            ]
        )
        self.logger = logging.getLogger("Existing114thProcessor")
    ...
    def _extract_metadata(self, root: ET.Element, xml_path: Path) -> Dict:
        ...
        if form is not None:
            ...
            # Extract just the number from "114th CONGRESS" -> 114
            if congress_text:
    ...
    def process_all_files(self):
        """Process all 114th Congress XML files in existing directory"""
        bills_dir = self.data_dir / "BILLS" / "bulkdata" / "BILLS" / "114"
        
        if not bills_dir.exists():
            self.logger.error(f"114th Congress directory not found: {bills_dir}")
            return
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Process existing 114th Congress XML files")
    ...
    processor = Existing114thProcessor(args.data_dir, args.db_path)
