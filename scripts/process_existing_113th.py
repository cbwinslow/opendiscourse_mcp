#!/usr/bin/env python3
"""
Process Existing 113th Congress XML Files
...
class Existing113thProcessor:
    ...
    def _setup_logging(self):
        ...
        logging.basicConfig(
            ...
            handlers=[
                ...
                logging.FileHandler("logs/existing_113th_processor.log", mode='a', encoding='utf-8')
            ]
        )
        self.logger = logging.getLogger("Existing113thProcessor")
    ...
    def _extract_metadata(self, root: ET.Element, xml_path: Path) -> Dict:
        ...
        if form is not None:
            ...
            # Extract just the number from "113th CONGRESS" -> 113
            if congress_text:
    ...
    def process_all_files(self):
        """Process all 113th Congress XML files in existing directory"""
        bills_dir = self.data_dir / "BILLS" / "bulkdata" / "BILLS" / "113"
        
        if not bills_dir.exists():
            self.logger.error(f"113th Congress directory not found: {bills_dir}")
            return
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Process existing 113th Congress XML files")
    ...
    processor = Existing113thProcessor(args.data_dir, args.db_path)
